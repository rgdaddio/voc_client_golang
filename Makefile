...
voc_client_go_app: test_client.go
	...
	$(CC) -v -g -o voc_client_go_app test_client.go client_utilities.go client_database.go client_sender.go client_registration.go -I $(shell pwd)/output $(shell pwd)/output/github.com/mattn/libgo-sqlite3.a $(shell pwd)/output/golang.org/x/net/libcontext.a -pthread -ldl  
